<template>
         <v-container class="mbl_Bspace pt-0">
             <v-col class="black_txt text-center pt-0" v-show="serviceChildType">
                 ...
                 <h3 class="font-weight-bold text-center text-uppercase mb-6 small_device_title">mobile reload</h3>

                 <h5>Please select mobile reload option</h5>

                 <v-col class="text-center">
                     <v-col class="tabs_bg py-2 d-inline-block" style="width:580px">
                         <v-row>
                             <v-col class="py-0 pl-2 pr-2">
                                 <a class="mbl_ser_tab" :class="prepaid==true ? 'activeBtn' : 'inActiveBtn'" @click="showOperator('prepaid')">Prepaid</a>
                             </v-col>
                             <!-- <v-col class="py-0 pr-2 pl-1">
                                  <a class="mbl_ser_tab" :class="postpaid==true ? 'activeBtn' : 'inActiveBtn'" v-on:click="showOperator('postpaid')">Postpaid</a>
                             </v-col> -->
                         </v-row>
                     </v-col>
                 </v-col>
              ...
              <v-row class="mb-6">
                  <v-col cols="12" md="4" class="pt-2 position-relative">
                    ...
                    <cool-select
                        ...
                        :placeholder="mobileReload ? '' : cryptoplaceholder"
                        ...
                        <template slot="item" slot-scope="{ item: mobileReloadOption }">
                            <div v-on:click="selCrypto()" style="display: flex; align-items: center;" v-if="mobileReloadOption.crypto_name!=''">
                            <img  v-bind:src="`${adminUrl}/coin_listing_images/${mobileReloadOption.coin_image}`" class="cryptoLogo">

                            <div>
                                
                                <h3>{{ mobileReloadOption.crypto_name }}</h3>
                                <p>{{ mobileReloadOption.balance }}</p>
                                <p>{{ mobileReloadOption.usd_balance }}</p>
                            </div>
                            </div>
                            <div v-if="mobileReloadOption.crypto_name==''">{{ mobileReloadOption.crypto_symbol }}</div>
                        </template>
                        <!-- slot for the selected item (in the text field) -->
                        <template slot="selection" slot-scope="{ item: mobileReloadOption }">
                            <img v-if="mobileReloadOption.crypto_name!=''" v-bind:src="`${adminUrl}/coin_listing_images/${mobileReloadOption.coin_image}`" class="cryptoLogo">
                   ...
                   <v-col cols="12" md="4" class="pt-2 position-relative">
                    ...
                    <cool-select
                        v-model="operator"
                        :items="operatorsList"
                        :placeholder="operator ? '' : 'CHOOSE YOUR OPERATOR'"
...
$(function () {
  ...
  methods: {
    ...
    showTransactionForm: function(product){
      ...
      let auth_token = this.$store.getters.authToken;
      if (auth_token) {
          const URL = process.env.VUE_APP_API_URL + "getBrexcoConfirmDet/"+self.mobileReload+"/"+product.amount+"/"+product.currency_type;
          axios({
              method: 'get',
              url: URL,
              headers: {
          ...
          .then(function (response) {
            ...
            self.crypto_amount=response.data.Result.crypto_amount+" "+self.mobileReload;
            self.blockchain_fee=response.data.Result.blockchain_fee+" "+self.mobileReload;
            ...
            self.discount=response.data.Result.discount+" "+self.mobileReload;
            self.total_amount=response.data.Result.amount_with_fee+" "+self.mobileReload;
            self.amount_with_fee=response.data.Result.amount_with_discount+" "+self.mobileReload;
    ...
    getAttributeLength: function(){
     ...
     if(this.mobileReload==null || this.mobileReload=="" || this.mobileReload=='Choose Payment Option'){
       this.cryptoerrclass="cryptofeilderr";
        this.isselcrypto=true;
        this.cryptoclass="";
      }
     if(this.operator=="" || this.operator==null ){
      ...
      this.mblnumselclass="";
       ...
       if(this.mobileReload!="" && this.mobileReload!=null && this.operator!='' && this.operator!=null ){
    ...
    // gets a flag pictur
    getFlag(mobileReload_crypto,adminUrl) {
      try {
        //alert();
        return require(`${adminUrl}/coin_listing_images/${mobileReload_crypto.toLowerCase()}`);
      } catch (e) {
   ...
   getOperator(mobileReload_operator) {
      try {
        return `https://operator-logo.dtone.com/logo-${mobileReload_operator}-1.png`;
      } catch (e) {
        return require("../../../assets/images/crypto/undefined.svg");
      } 
    },
