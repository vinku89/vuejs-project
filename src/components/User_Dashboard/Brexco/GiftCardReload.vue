<template>
         ...
         <v-container class="mbl_Bspace pt-0">
             <v-col class="black_txt text-center pt-0" v-show="serviceChildType">
                 ...
                 <h3 class="font-weight-bold text-center text-uppercase mb-6 small_device_title">Gift Cards</h3>

                 <h5>Please select Gift Cards option</h5>
            ...
            <v-col v-show="operatorSel" >
              <v-row class="mb-6">
                  <v-col cols="12" md="4" class="pt-2 position-relative">
                    ...
                    <cool-select
                        v-model="giftCardReload"
                        :items="cryptoPaymentsList"
                        :placeholder="giftCardReload ? '' : cryptoplaceholder"
                        ...
                        <template slot="item" slot-scope="{ item: giftCardReloadOption }">
                            <div v-on:click="selCrypto()" style="display: flex; align-items: center;" v-if="giftCardReloadOption.crypto_name!=''">
                            <img  v-bind:src="`${adminUrl}/coin_listing_images/${giftCardReloadOption.coin_image}`" class="cryptoLogo">

                            <div>
                                
                                <h3>{{ giftCardReloadOption.crypto_name }}</h3>
                                <p>{{ giftCardReloadOption.balance }}</p>
                                <p>{{ giftCardReloadOption.usd_balance }}</p>
                            </div>
                            </div>
                            <div v-if="giftCardReloadOption.crypto_name==''">{{ giftCardReloadOption.crypto_symbol }}</div>
                        </template>
                        <!-- slot for the selected item (in the text field) -->
                        <template slot="selection" slot-scope="{ item: giftCardReloadOption }">
                            <img v-if="giftCardReloadOption.crypto_name!=''" v-bind:src="`${adminUrl}/coin_listing_images/${giftCardReloadOption.coin_image}`" class="cryptoLogo">
                   ...
                   <v-col cols="12" md="4" class="pt-2 position-relative">
                    <h5 class="black_txt mb-4"><sup class="red_txt">*</sup>Please choose your Gift Cards</h5>
                    <cool-select
                        v-model="operator"
                        :items="operatorsList"
                        :placeholder="operator ? '' : 'Select Gift Card'"
              ...
              <v-row justify="center">
                <v-dialog v-model="paymentCompleted" persistent max-width="500" content-class="confirmation2fa">
                  <v-card class="text-center">
                    ...
                    <p class="mb-7 f18">for Gift Cards</p>
...
$(function () {
  ...
  methods: {
    ...
    showTransactionForm: function(product){
      ...
      let auth_token = this.$store.getters.authToken;
      if (auth_token) {
          const URL = process.env.VUE_APP_API_URL + "getBrexcoConfirmDet/"+self.giftCardReload+"/"+product.amount+"/"+product.currency_type;
          axios({
              method: 'get',
              url: URL,
              headers: {
          ...
          .then(function (response) {
            ...
            self.crypto_amount=response.data.Result.crypto_amount+" "+self.giftCardReload;
            self.blockchain_fee=response.data.Result.blockchain_fee+" "+self.giftCardReload;
            ...
            self.discount=response.data.Result.discount+" "+self.giftCardReload;
            self.total_amount=response.data.Result.amount_with_fee+" "+self.giftCardReload;
            self.amount_with_fee=response.data.Result.amount_with_discount+" "+self.giftCardReload;
    ...
    getAttributeLength: function(){
     ...
     if(this.giftCardReload==null || this.giftCardReload=="" || this.giftCardReload=='Choose Payment Option'){
       this.cryptoerrclass="cryptofeilderr";
        this.isselcrypto=true;
        this.cryptoclass="";
      }
     if(this.operator=="" || this.operator==null ){
      ...
      this.mblnumselclass="";
       ...
       if(this.giftCardReload!="" && this.giftCardReload!=null && this.operator!='' && this.operator!=null ){
    ...
    // gets a flag pictur
    getFlag(giftCardReload_crypto,adminUrl) {
      try {
        //alert();
        return require(`${adminUrl}/coin_listing_images/${giftCardReload_crypto.toLowerCase()}`);
      } catch (e) {
        return require("../../../assets/images/crypto/undefined.svg");
      } 
    },
   getOperator(giftCardReload_operator) {
      try {
        return `https://operator-logo.dtone.com/logo-${giftCardReload_operator}-1.png`;
      } catch (e) {
        return require("../../../assets/images/crypto/undefined.svg");
      } 
    },
