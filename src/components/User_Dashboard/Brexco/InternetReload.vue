<template>
         ...
         <v-container class="mbl_Bspace pt-0">
             <v-col class="black_txt text-center pt-0" v-show="serviceChildType">
                 ...
                 <h3 class="font-weight-bold text-center text-uppercase mb-6 small_device_title">Internet</h3>

                 <h5>Please select Internet option</h5>
            ...
            <v-col v-show="operatorSel" >
              <v-row class="mb-6">
                  <v-col cols="12" md="4" class="pt-2 position-relative">
                    ...
                    <cool-select
                        v-model="internetReload"
                        :items="cryptoPaymentsList"
                        :placeholder="internetReload ? '' : cryptoplaceholder"
                        ...
                        <template slot="item" slot-scope="{ item: internetReloadOption }">
                            <div v-on:click="selCrypto()" style="display: flex; align-items: center;" v-if="internetReloadOption.crypto_name!=''">
                            <img  v-bind:src="`${adminUrl}/coin_listing_images/${internetReloadOption.coin_image}`" class="cryptoLogo">

                            <div>
                                
                                <h3>{{ internetReloadOption.crypto_name }}</h3>
                                <p>{{ internetReloadOption.balance }}</p>
                                <p>{{ internetReloadOption.usd_balance }}</p>
                            </div>
                            </div>
                            <div v-if="internetReloadOption.crypto_name==''">{{ internetReloadOption.crypto_symbol }}</div>
                        </template>
                        <!-- slot for the selected item (in the text field) -->
                        <template slot="selection" slot-scope="{ item: internetReloadOption }">
                            <img v-if="internetReloadOption.crypto_name!=''" v-bind:src="`${adminUrl}/coin_listing_images/${internetReloadOption.coin_image}`" class="cryptoLogo">
                   ...
                   <v-col cols="12" md="4" class="pt-2 position-relative">
                    <h5 class="black_txt mb-4"><sup class="red_txt">*</sup>Please choose your Internet Operator</h5>
                    <cool-select
                        v-model="operator"
                        :items="operatorsList"
                        :placeholder="operator ? '' : 'Select Internet Operator'"
              ...
              <v-row justify="center">
                <v-dialog v-model="paymentCompleted" persistent max-width="500" content-class="confirmation2fa">
                  <v-card class="text-center">
                    ...
                    <p class="mb-7 f18">for Internet</p>
...
$(function () {
  ...
  methods: {
    ...
    showTransactionForm: function(product){
      ...
      let auth_token = this.$store.getters.authToken;
      if (auth_token) {
          const URL = process.env.VUE_APP_API_URL + "getBrexcoConfirmDet/"+self.internetReload+"/"+product.amount+"/"+product.currency_type;
          axios({
              method: 'get',
              url: URL,
              headers: {
          ...
          .then(function (response) {
            ...
            self.crypto_amount=response.data.Result.crypto_amount+" "+self.internetReload;
            self.blockchain_fee=response.data.Result.blockchain_fee+" "+self.internetReload;
            ...
            self.discount=response.data.Result.discount+" "+self.internetReload;
            self.total_amount=response.data.Result.amount_with_fee+" "+self.internetReload;
            self.amount_with_fee=response.data.Result.amount_with_discount+" "+self.internetReload;
    ...
    getAttributeLength: function(){
     ...
     if(this.internetReload==null || this.internetReload=="" || this.internetReload=='Choose Payment Option'){
       this.cryptoerrclass="cryptofeilderr";
        this.isselcrypto=true;
        this.cryptoclass="";
      }
     if(this.operator=="" || this.operator==null ){
      ...
      this.mblnumselclass="";
       ...
       if(this.internetReload!="" && this.internetReload!=null && this.operator!='' && this.operator!=null ){
        if(mobileNumber>0){
          this.mblnoError="";
          this.havemobileNumber=false;
    ...
    // gets a flag pictur
    getFlag(internetReload_crypto,adminUrl) {
      try {
        //alert();
        return require(`${adminUrl}/coin_listing_images/${internetReload_crypto.toLowerCase()}`);
      } catch (e) {
        return require("../../../assets/images/crypto/undefined.svg");
      } 
    },
   getOperator(internetReload_operator) {
      try {
        return `https://operator-logo.dtone.com/logo-${internetReload_operator}-1.png`;
      } catch (e) {
