<template>
         ...
         <v-container class="mbl_Bspace pt-0">
             <v-col class="black_txt text-center pt-0" v-show="serviceChildType">
                 ...
                 <h3 class="font-weight-bold text-center text-uppercase mb-6 small_device_title">Utilities</h3>

                 <h5>Please select Utilities option</h5>
            ...
            <v-col v-show="operatorSel" >
              <v-row class="mb-6">
                  <v-col cols="12" md="4" class="pt-2 position-relative">
                    ...
                    <cool-select
                        v-model="utilitiesReload"
                        :items="cryptoPaymentsList"
                        :placeholder="utilitiesReload ? '' : cryptoplaceholder"
                        ...
                        <template slot="item" slot-scope="{ item: utilitiesReloadOption }">
                            <div v-on:click="selCrypto()" style="display: flex; align-items: center;" v-if="utilitiesReloadOption.crypto_name!=''">
                            <img  v-bind:src="`${adminUrl}/coin_listing_images/${utilitiesReloadOption.coin_image}`" class="cryptoLogo">

                            <div>
                                
                                <h3>{{ utilitiesReloadOption.crypto_name }}</h3>
                                <p>{{ utilitiesReloadOption.balance }}</p>
                                <p>{{ utilitiesReloadOption.usd_balance }}</p>
                            </div>
                            </div>
                            <div v-if="utilitiesReloadOption.crypto_name==''">{{ utilitiesReloadOption.crypto_symbol }}</div>
                        </template>
                        <!-- slot for the selected item (in the text field) -->
                        <template slot="selection" slot-scope="{ item: utilitiesReloadOption }">
                            <img v-if="utilitiesReloadOption.crypto_name!=''" v-bind:src="`${adminUrl}/coin_listing_images/${utilitiesReloadOption.coin_image}`" class="cryptoLogo">
                   ...
                   <v-col cols="12" md="4" class="pt-2 position-relative">
                    <h5 class="black_txt mb-4"><sup class="red_txt">*</sup>Please choose your Operator</h5>
                    <cool-select
                        v-model="operator"
                        :items="operatorsList"
                        :placeholder="operator ? '' : 'Select Operator'"
              ...
              <v-row justify="center">
                <v-dialog v-model="paymentCompleted" persistent max-width="500" content-class="confirmation2fa">
                  <v-card class="text-center">
                    ...
                    <p class="mb-7 f18">for Utilities</p>
...
$(function () {
  ...
  methods: {
    ...
    showTransactionForm: function(product){
      ...
      let auth_token = this.$store.getters.authToken;
      if (auth_token) {
          const URL = process.env.VUE_APP_API_URL + "getBrexcoConfirmDet/"+self.utilitiesReload+"/"+product.amount+"/"+product.currency_type;
          axios({
              method: 'get',
              url: URL,
              headers: {
          ...
          .then(function (response) {
            ...
            self.crypto_amount=response.data.Result.crypto_amount+" "+self.utilitiesReload;
            self.blockchain_fee=response.data.Result.blockchain_fee+" "+self.utilitiesReload;
            ...
            self.discount=response.data.Result.discount+" "+self.utilitiesReload;
            self.total_amount=response.data.Result.amount_with_fee+" "+self.utilitiesReload;
            self.amount_with_fee=response.data.Result.amount_with_discount+" "+self.utilitiesReload;
    ...
    getAttributeLength: function(){
     ...
     if(this.utilitiesReload==null || this.utilitiesReload=="" || this.utilitiesReload=='Choose Payment Option'){
       this.cryptoerrclass="cryptofeilderr";
        this.isselcrypto=true;
        this.cryptoclass="";
      }
     if(this.operator=="" || this.operator==null ){
      ...
      this.mblnumselclass="";
       ...
       if(this.utilitiesReload!="" && this.utilitiesReload!=null && this.operator!='' && this.operator!=null ){
        if(mobileNumber>0){
          this.mblnoError="";
          this.havemobileNumber=false;
    ...
    // gets a flag pictur
    getFlag(utilitiesReload_crypto,adminUrl) {
      try {
        //alert();
        return require(`${adminUrl}/coin_listing_images/${utilitiesReload_crypto.toLowerCase()}`);
      } catch (e) {
        return require("../../../assets/images/crypto/undefined.svg");
      } 
    },
   getOperator(utilitiesReload_operator) {
      try {
        return `https://operator-logo.dtone.com/logo-${utilitiesReload_operator}-1.png`;
      } catch (e) {
