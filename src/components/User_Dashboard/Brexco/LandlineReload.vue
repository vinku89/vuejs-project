<template>
         ...
         <v-container class="mbl_Bspace pt-0">
             <v-col class="black_txt text-center pt-0" v-show="serviceChildType">
                 ...
                 <h3 class="font-weight-bold text-center text-uppercase mb-6 small_device_title">Landline</h3>

                 <h5>Please select Landline option</h5>
            ...
            <v-col v-show="operatorSel" >
              <v-row class="mb-6">
                  <v-col cols="12" md="4" class="pt-2 position-relative">
                    ...
                    <cool-select
                        v-model="landlineReload"
                        :items="cryptoPaymentsList"
                        :placeholder="landlineReload ? '' : cryptoplaceholder"
                        ...
                        <template slot="item" slot-scope="{ item: landlineReloadOption }">
                            <div v-on:click="selCrypto()" style="display: flex; align-items: center;" v-if="landlineReloadOption.crypto_name!=''">
                            <img  v-bind:src="`${adminUrl}/coin_listing_images/${landlineReloadOption.coin_image}`" class="cryptoLogo">

                            <div>
                                
                                <h3>{{ landlineReloadOption.crypto_name }}</h3>
                                <p>{{ landlineReloadOption.balance }}</p>
                                <p>{{ landlineReloadOption.usd_balance }}</p>
                            </div>
                            </div>
                            <div v-if="landlineReloadOption.crypto_name==''">{{ landlineReloadOption.crypto_symbol }}</div>
                        </template>
                        <!-- slot for the selected item (in the text field) -->
                        <template slot="selection" slot-scope="{ item: landlineReloadOption }">
                            <img v-if="landlineReloadOption.crypto_name!=''" v-bind:src="`${adminUrl}/coin_listing_images/${landlineReloadOption.coin_image}`" class="cryptoLogo">
              ...
              <v-row justify="center">
                <v-dialog v-model="paymentCompleted" persistent max-width="500" content-class="confirmation2fa">
                  <v-card class="text-center">
                    ...
                    <p class="mb-7 f18">for Landline</p>
...
$(function () {
  ...
  methods: {
    ...
    showTransactionForm: function(product){
      ...
      let auth_token = this.$store.getters.authToken;
      if (auth_token) {
          const URL = process.env.VUE_APP_API_URL + "getBrexcoConfirmDet/"+self.landlineReload+"/"+product.amount+"/"+product.currency_type;
          axios({
              method: 'get',
              url: URL,
              headers: {
          ...
          .then(function (response) {
            ...
            self.crypto_amount=response.data.Result.crypto_amount+" "+self.landlineReload;
            self.blockchain_fee=response.data.Result.blockchain_fee+" "+self.landlineReload;
            ...
            self.discount=response.data.Result.discount+" "+self.landlineReload;
            self.total_amount=response.data.Result.amount_with_fee+" "+self.landlineReload;
            self.amount_with_fee=response.data.Result.amount_with_discount+" "+self.landlineReload;
    ...
    getAttributeLength: function(){
     ...
     if(this.landlineReload==null || this.landlineReload=="" || this.landlineReload=='Choose Payment Option'){
       this.cryptoerrclass="cryptofeilderr";
        this.isselcrypto=true;
        this.cryptoclass="";
      }
     if(this.operator=="" || this.operator==null ){
      ...
      this.mblnumselclass="";
       ...
       if(this.landlineReload!="" && this.landlineReload!=null && this.operator!='' && this.operator!=null ){
        if(mobileNumber>0){
          this.mblnoError="";
          this.havemobileNumber=false;
    ...
    // gets a flag pictur
    getFlag(landlineReload_crypto,adminUrl) {
      try {
        //alert();
        return require(`${adminUrl}/coin_listing_images/${landlineReload_crypto.toLowerCase()}`);
      } catch (e) {
        return require("../../../assets/images/crypto/undefined.svg");
      } 
    },
   getOperator(landlineReload_operator) {
      try {
        return `https://operator-logo.dtone.com/logo-${landlineReload_operator}-1.png`;
      } catch (e) {
