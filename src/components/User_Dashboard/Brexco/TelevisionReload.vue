<template>
         ...
         <v-container class="mbl_Bspace pt-0">
             <v-col class="black_txt text-center pt-0" v-show="serviceChildType">
                 ...
                 <h3 class="font-weight-bold text-center text-uppercase mb-6 small_device_title">TELEVISION</h3>

                 <h5>Please select TELEVISION option</h5>
            ...
            <v-col v-show="operatorSel" >
              <v-row class="mb-6">
                  <v-col cols="12" md="4" class="pt-2 position-relative">
                    ...
                    <cool-select
                        v-model="televisionReload"
                        :items="cryptoPaymentsList"
                        :placeholder="televisionReload ? '' : cryptoplaceholder"
                        ...
                        <template slot="item" slot-scope="{ item: televisionReloadOption }">
                            <div v-on:click="selCrypto()" style="display: flex; align-items: center;" v-if="televisionReloadOption.crypto_name!=''">
                            <img  v-bind:src="`${adminUrl}/coin_listing_images/${televisionReloadOption.coin_image}`" class="cryptoLogo">

                            <div>
                                
                                <h3>{{ televisionReloadOption.crypto_name }}</h3>
                                <p>{{ televisionReloadOption.balance }}</p>
                                <p>{{ televisionReloadOption.usd_balance }}</p>
                            </div>
                            </div>
                            <div v-if="televisionReloadOption.crypto_name==''">{{ televisionReloadOption.crypto_symbol }}</div>
                        </template>
                        <!-- slot for the selected item (in the text field) -->
                        <template slot="selection" slot-scope="{ item: televisionReloadOption }">
                            <img v-if="televisionReloadOption.crypto_name!=''" v-bind:src="`${adminUrl}/coin_listing_images/${televisionReloadOption.coin_image}`" class="cryptoLogo">
                   ...
                   <v-col cols="12" md="4" class="pt-2 position-relative">
                    <h5 class="black_txt mb-4"><sup class="red_txt">*</sup>Please choose your DTH Operator</h5>
                    <cool-select
                        v-model="operator"
                        :items="operatorsList"
                        :placeholder="operator ? '' : 'Select DTH Operator'"
                  ...
                  <v-col cols="12" md="4">
                    ...
                    <div class="position-relative d-inline-block" :class="mblnumselclass">
                      ...
                      <div class="error_msg w-100 " style="top:3px" v-show="havemobileNumber">
                        {{mbl_err_txt}}
                      </div>
                    </div>
                  </v-col>
                  <v-col cols="12" md="4">
                    <h5 class="black_txt mb-4 text-left"><sup class="red_txt">*</sup>Your Account number</h5>
                    <div class="position-relative" :class="mblnumselclass">
                      <v-text-field
                        @paste="checkAlphaNumber($event)"
                        outlined
                        single-line
                        class="account_no"
                        placeholder="ACCOUNT NUMBER"
                        hide-details
                        @keypress="isAlphaNumber($event)"
              ...
              <v-col md="10" offset-md="1">
                <v-row>
                   ...
                   <v-col  cols="12" xl="4" lg="6" md="6" sm="6" v-for="product in products" :key="product.product_id">
                     <div class="avl_amt" v-on:click="showTransactionForm(product)" v-scroll-to="{ el: '#confirm_div' }">
             ...
             <v-col class="payment_calc_wrp">
                <table class="table">
                  <tbody>
                    ...
                    <tr>
                      <td>Mobile Number</td>
                      <td class="text-right">{{countryCode}} {{mobileNumber}}</td>
                    </tr>
                     <tr>
              ...
              <v-row justify="center">
                <v-dialog v-model="paymentCompleted" persistent max-width="500" content-class="confirmation2fa">
                  <v-card class="text-center">
                    ...
                    <p class="mb-7 f18">for Television</p>
...
export default {
  ...
  data: () => ({
    ...
    acct_err_txt:"Please enter Account number",
    haveaccountNumber:false,
    havemobileNumber:false,
    isselcrypto:false,
    isseloperator:false,
    confirmbtn:false,
    absolute:true,
...
$(function () {
  ...
  methods: {
    ...
    showTransactionForm: function(product){
      ...
      let auth_token = this.$store.getters.authToken;
      if (auth_token) {
          const URL = process.env.VUE_APP_API_URL + "getBrexcoConfirmDet/"+self.televisionReload+"/"+product.amount+"/"+product.currency_type;
          axios({
              method: 'get',
              url: URL,
              headers: {
          ...
          .then(function (response) {
            ...
            self.crypto_amount=response.data.Result.crypto_amount+" "+self.televisionReload;
            self.blockchain_fee=response.data.Result.blockchain_fee+" "+self.televisionReload;
            ...
            self.discount=response.data.Result.discount+" "+self.televisionReload;
            self.total_amount=response.data.Result.amount_with_fee+" "+self.televisionReload;
            self.amount_with_fee=response.data.Result.amount_with_discount+" "+self.televisionReload;
    ...
    //   $('html, body').animate({scrollTop: $("#confirm_div").offset().top }, 2000);
    // },
    check2fa: function(){
    ...
    checkAlphaNumber:function(evt) {
      evt.preventDefault();
      //alert(evt);
      let amt = evt.clipboardData.getData('text');
            let bamt = amt.replace(/\s/g,"");
            let mnum = bamt.replace(/,/g, '');
      this.accountNumber = mnum.replace(/[^0-9A-Za-z]/g, '');
      //alert(this.accountNumber);
      if(this.accountNumber.length>15){
        this.accountNumber=this.accountNumber.substring(0,15);
      }
    },
    checkNumber2:function(evt) {
      evt.preventDefault();
      //alert(evt);
      let amt = evt.clipboardData.getData('text');
            let bamt = amt.replace(/\s/g,"");
            let mnum = bamt.replace(/,/g, '');
      this.mobileNumber = mnum.replace(/[^0-9]/g, '');
      //alert(this.accountNumber);
      if(this.mobileNumber.length>15){
        this.mobileNumber=this.mobileNumber.substring(0,15);
      }
    },
    isAlphaNumber: function(evt) {
        if(this.accountNumber.length>=15){
          evt.preventDefault();
        }
        evt = (evt) ? evt : window.event;
        var charCode = (evt.which) ? evt.which : evt.keyCode;
        if ((charCode > 31 && ((charCode < 48 || charCode > 57) && (charCode < 65 || charCode > 91) && (charCode < 96 || charCode > 122))) && charCode !== 46) {
            evt.preventDefault();
        } else if (charCode === 46) {
            evt.preventDefault();
        } else {
    ...
    getAttributeLength: function(){
      ...
      if(accountNumber=="" || accountNumber==0){
        this.acctnoError="acctnoErrorclass";
      ...
      if(mobileNumber=="" || mobileNumber==0){
        ...
        this.havemobileNumber=true;
      }else{
        this.mblnoError="";
        this.havemobileNumber=false;
      }
     if(this.televisionReload==null || this.televisionReload=="" || this.televisionReload=='Choose Payment Option'){
       this.cryptoerrclass="cryptofeilderr";
        this.isselcrypto=true;
        this.cryptoclass="";
      }
     if(this.operator=="" || this.operator==null ){
      ...
      this.mblnumselclass="";
       ...
       if(this.televisionReload!="" && this.televisionReload!=null && this.operator!='' && this.operator!=null ){
        if(mobileNumber>0){
          this.mblnoError="";
          this.havemobileNumber=false;
        ...
        if(accountNumber.length>0){
          if(!isNaN(parseFloat(accountNumber)) && isFinite(accountNumber)){
            if(accountNumber>0){
              this.acctnoError="";
              this.haveaccountNumber=false;
              this.mblnumselclass="selmblnumber";
          ...
          this.acct_err_txt="Please enter valid account number";      
        }

        if(mobileNumber>0 && accountNumber.length>0){
          if(!isNaN(parseFloat(accountNumber)) && isFinite(accountNumber)) {
            if(accountNumber>0){
              this.mobilefieldact=false;
            }
    ...
    // gets a flag pictur
    getFlag(televisionReload_crypto,adminUrl) {
      try {
        //alert();
        return require(`${adminUrl}/coin_listing_images/${televisionReload_crypto.toLowerCase()}`);
      } catch (e) {
        return require("../../../assets/images/crypto/undefined.svg");
      } 
    },
   getOperator(televisionReload_operator) {
      try {
        return `https://operator-logo.dtone.com/logo-${televisionReload_operator}-1.png`;
      } catch (e) {
